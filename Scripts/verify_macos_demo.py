...
from verification_modes import VerificationMode, VerificationPhase
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="codexkit-demo-verification-") as temporary:
        ...
        if args.mode == VerificationMode.SMOKE:
            command.append("--verify-smoke")
        for phase in VerificationPhase:
            result_path.unlink(missing_ok=True)
            extra = ["--verify-recovery-reopen"] if phase == VerificationPhase.REOPEN else []
            ...
            report = "recovery-reopen-result.json" if phase == VerificationPhase.REOPEN else "verification-result.json"
