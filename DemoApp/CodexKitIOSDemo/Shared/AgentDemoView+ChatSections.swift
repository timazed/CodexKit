    ...
    var header: some View {
        DemoSectionCard {
            ...
            if let session = viewModel.session {
                DemoAccountDetailsView(account: session.account)
            }
            ...
            Picker(
                "Persistence Adapter",
                selection: Binding(
                    get: { viewModel.persistenceAdapter },
                    set: { adapter in
                        Task {
                            await viewModel.updatePersistenceAdapter(adapter)
                        }
                    }
                )
            ) {
                ForEach(DemoPersistenceAdapter.allCases) { adapter in
            ...
            Text("Switching changes both runtime-state and memory stores. SQLite and Realm keep separate files, so you can exercise either adapter without overwriting the other.")
                .font(.caption)
                .foregroundStyle(.secondary)
        ...
        DemoSectionCard {
            HStack {
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Text(
                        viewModel.session == nil
                            ? "Persisted threads are still available. Sign in to resume one."
                            : "Pick a thread to open its transcript, attachments, and composer in a dedicated view."
                    )
                        .font(.subheadline)
                        .foregroundStyle(.secondary)
                }
