...
- includes a `Parallel Lookups` quick start whose skill allows two sample tools, two calls in one round, and up to two concurrent calls
...
3. Under **Quick Starts**, tap **Parallel Lookups**, then **Open Current Thread**. The demo attaches a skill that allows only the weather and transport sample tools, with two host calls in one round and up to two concurrent calls. The activity card lists running tools and retains the peak overlap after completion. Both tools opt into parallel execution and pause briefly to make overlap visible. Weather and transport results are fixed sample data. The model chooses which calls to request; a request for another tool round receives a normal budget failure, and the peak-overlap label appears only when overlap actually occurred. The macOS **Runtime → Parallel Lookups** example uses the same limits and reports the observed peak alongside them.
