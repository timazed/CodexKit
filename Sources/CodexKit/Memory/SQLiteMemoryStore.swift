...
public actor SQLiteMemoryStore: MemoryStoring {
    ...
    public func put(_ record: MemoryRecord) async throws {
        ...
        try await writeTransaction { db in
            try Self.ensureRecordIDAvailable(record.id, namespace: record.namespace, in: db)
            if let dedupeKey = record.dedupeKey {
                try Self.ensureDedupeKeyAvailable(dedupeKey, namespace: record.namespace, in: db)
            }
            try Self.upsertRecord(record, in: db)
        }
    }
    ...
    public func upsert(_ record: MemoryRecord, dedupeKey: String) async throws {
        try MemoryQueryEngine.validateNamespace(record.namespace)
        try await writeTransaction { db in
            try Self.deleteRecord(withDedupeKey: dedupeKey, namespace: record.namespace, in: db)
            try Self.deleteRecord(id: record.id, namespace: record.namespace, in: db)
            var updatedRecord = record
            updatedRecord.dedupeKey = dedupeKey
            try Self.upsertRecord(updatedRecord, in: db)
        }
    }
    ...
    public func query(_ query: MemoryQuery) async throws -> MemoryQueryResult {
        ...
        let records = try await dbQueue.read { db in
            try Self.loadRecords(namespace: query.namespace, in: db)
        }
        let rawScores = try await dbQueue.read { db in
            try Self.loadFTSRawScores(
    ...
    public func record(
        ...
        return try await dbQueue.read { db in
            try Self.loadRecords(namespace: namespace, in: db).first { $0.id == id }
        }
    }
    ...
    public func diagnostics(namespace: String) async throws -> MemoryStoreDiagnostics {
        try MemoryQueryEngine.validateNamespace(namespace)
        let records = try await dbQueue.read { db in
            try Self.loadRecords(namespace: namespace, in: db)
        }
    ...
    public func compact(_ request: MemoryCompactionRequest) async throws {
        ...
        try await writeTransaction { db in
            try Self.ensureRecordIDAvailable(
                request.replacement.id,
                namespace: request.replacement.namespace,
                in: db
            )
            if let dedupeKey = request.replacement.dedupeKey {
                try Self.ensureDedupeKeyAvailable(
                    dedupeKey,
                    namespace: request.replacement.namespace,
                    in: db
                )
            }
            try Self.upsertRecord(request.replacement, in: db)
            for sourceID in request.sourceIDs {
                try Self.archiveRecord(id: sourceID, namespace: request.replacement.namespace, in: db)
            }
        }
    }
    ...
    public func archive(ids: [String], namespace: String) async throws {
        ...
        try await writeTransaction { db in
            for id in ids {
                try Self.archiveRecord(id: id, namespace: namespace, in: db)
            }
        }
    }
    ...
    public func delete(ids: [String], namespace: String) async throws {
        ...
        try await writeTransaction { db in
            for id in ids {
                try Self.deleteRecord(id: id, namespace: namespace, in: db)
            }
        }
    }
    ...
    private static func ensureRecordIDAvailable(
        _ id: String,
        namespace: String,
        in db: Database
    ) throws {
    ...
    private static func ensureDedupeKeyAvailable(
        _ dedupeKey: String,
        namespace: String,
        in db: Database
    ) throws {
    ...
    private static func loadRecords(
        ...
        return try rows.map { row in
            MemoryRecord(
                ...
                kind: row["kind"],
                summary: row["summary"],
                evidence: try decodeJSON([String].self, from: row["evidence_json"]),
                ...
                tags: try decodeJSON([String].self, from: row["tags_json"]),
                relatedIDs: try decodeJSON([String].self, from: row["related_ids_json"]),
                dedupeKey: row["dedupe_key"],
                isPinned: (row["is_pinned"] as Int64? ?? 0) == 1,
                attributes: try decodeNullableJSON(JSONValue.self, from: row["attributes_json"]),
                status: MemoryRecordStatus(rawValue: row["status"]) ?? .active
            )
        }
    }

    private static func loadFTSRawScores(
        namespace: String,
        queryText: String?,
        in db: Database
    ) throws -> [String: Double] {
        let matchQuery = Self.ftsQuery(from: queryText)
    ...
    private static func archiveRecord(
        id: String,
        namespace: String,
        in db: Database
    ) throws {
    ...
    private static func upsertRecord(
        ...
        try db.execute(
            ...
            arguments: [
                ...
                record.kind,
                record.summary,
                try encodeJSON(record.evidence),
                record.importance,
                record.createdAt.timeIntervalSince1970,
                record.observedAt?.timeIntervalSince1970,
                record.expiresAt?.timeIntervalSince1970,
                try encodeJSON(record.tags),
                try encodeJSON(record.relatedIDs),
                record.dedupeKey,
                record.isPinned ? 1 : 0,
                try encodeNullableJSON(record.attributes),
    ...
    private static func encodeNullableJSON<T: Encodable>(_ value: T?) throws -> String? {
    ...
    private static func decodeNullableJSON<T: Decodable>(
        _ type: T.Type,
        from string: String?
    ) throws -> T? {
        ...
        return try Self.decodeJSON(type, from: string)
    }

    private static func ftsQuery(from value: String?) -> String {
    ...
    private static func schemaVersion(in db: Database) throws -> Int {
