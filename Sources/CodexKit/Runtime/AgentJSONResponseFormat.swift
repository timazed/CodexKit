...
public struct AgentJSONResponseFormat<Output: Codable & Sendable>: AgentOutputFormat {
    ...
    public init(name: String, description: String? = nil, schema: JSONSchema,
        ...
        self.limits = limits; self.finalValidation = finalValidation
    }
    ...
    public var formatInstructions: String { "Finish tool work before emitting the final JSON answer. Do not restart or revise the answer after it begins." }
...
public actor AgentJSONOutputDecoder<Output: Codable & Sendable>: AgentOutputDecoder {
    ...
    init(schema: JSONSchema, limits: AgentStructuredOutputLimits) { self.schema = schema; self.limits = limits }
    ...
    public func consume(_ bytes: Data, into sink: AgentOutputEventSink<Event>) async throws {
        ...
        guard bytes.count <= limits.maximumInputBytes - source.count else { throw AgentOutputError.limit("JSON input exceeds limit.") }
        source.append(bytes)
        var buffer = text
...
struct AgentOutputUTF8Buffer: Sendable {
    ...
    mutating func consume(_ bytes: Data, maximumChunk: Int,
                          emit: @Sendable (String) async throws -> Void) async throws {
        guard maximumChunk >= 4 else { throw AgentOutputError.invalidFormat("Text events require a byte limit of at least four.") }
        var offset = 0
        while offset < bytes.count {
            try Task.checkCancellation()
            let count = min(maximumChunk - pending.count, bytes.count - offset)
            pending.append(contentsOf: bytes[offset ..< offset + count]); offset += count
            ...
            for suffix in 0 ... min(3, pending.count) {
                ...
                if String(data: prefix, encoding: .utf8) != nil {
                    decoded = (String(decoding: prefix, as: UTF8.self), pending.count - suffix); break
                }
            }
            guard let (text, length) = decoded else { throw AgentOutputError.invalidOutput("Output is not valid UTF-8.") }
            if length > 0 {
