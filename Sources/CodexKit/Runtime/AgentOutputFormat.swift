...
public struct AgentStructuredOutputLimits: Codable, Hashable, Sendable {
    ...
    func validate() throws {
        guard maximumInputBytes > 0, maximumInputBytes <= AgentStoreLimits.maximumMessageTextByteCount,
              ...
              maximumSchemaBytes <= 1_048_576 else { throw AgentOutputError.limit("Invalid structured-output limits.") }
    }
...
public enum AgentOutputError: Error, LocalizedError, Sendable {
    ...
    public var errorDescription: String? {
        switch self {
        case let .invalidFormat(message), let .invalidOutput(message), let .protocolViolation(message), let .limit(message): message
...
public struct AgentOutputFailure: Sendable {
    ...
    public init(message: String, recordIndex: Int? = nil) {
...
public struct AgentOutputPersistence<Output: Sendable>: Sendable {
    ...
    public init(encode: @escaping @Sendable (Output) throws -> Data,
...
public protocol AgentOutputFormat: Sendable {
    ...
    var schemaRepresentation: String? { get }
    var limits: AgentStructuredOutputLimits { get }
    var persistence: AgentOutputPersistence<Decoder.Output>? { get }
    func makeDecoder() throws -> Decoder
    func validateFinal(_ output: Decoder.Output) async throws
}
...
public struct AgentOutputRepresentation: Codable, Hashable, Sendable {
    ...
    public func restore<Format: AgentOutputFormat>(using format: Format) throws -> Format.Decoder.Output {
        guard envelopeVersion == 1, codecIdentifier == format.codecIdentifier,
              formatVersion == format.formatVersion, schema == format.schemaRepresentation else {
            throw AgentOutputError.unsupportedVersion
        }
        guard let persistence = format.persistence else { throw AgentOutputError.persistenceUnavailable }
        guard encodedOutput.count <= format.limits.maximumOutputBytes,
              rawText.utf8.count <= format.limits.maximumInputBytes else { throw AgentOutputError.limit("Stored output exceeds limits.") }
