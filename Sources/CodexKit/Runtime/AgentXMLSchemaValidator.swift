...
final class AgentXMLSchemaValidator: @unchecked Sendable {
    ...
    init(xsd: String, root: XMLName, limits: AgentStructuredOutputLimits) throws {
        guard xsd.utf8.count <= limits.maximumSchemaBytes else {
            throw AgentOutputError.limit("XSD exceeds schema byte limit.")
        }
        try XMLSchemaCompiler.validName(root.localName)
        // XSD syntax is deeper and has more nodes than the response it describes.
        // Only the schema byte budget comes from the response configuration.
        var schemaLimits = AgentStructuredOutputLimits()
        schemaLimits.maximumInputBytes = limits.maximumSchemaBytes
        let preflight = try Self.parse(Data(xsd.utf8), limits: schemaLimits)
        ...
        guard preflight.name == XMLName("schema", namespaceURI: xs),
            ...
            throw AgentOutputError.invalidFormat(
                "XSD must declare the selected root and target namespace and use XSD 1.0.")
        }
        ...
        guard let context = xmlSchemaNewDocParserCtxt(doc) else {
            throw AgentOutputError.invalidFormat("Could not allocate XSD context.")
        }
        defer { xmlSchemaFreeParserCtxt(context) }
        xmlSchemaSetParserStructuredErrors(context, { _, _ in }, nil)
        guard let schema = xmlSchemaParse(context) else {
            throw AgentOutputError.invalidFormat("XSD compilation failed: unsupported or invalid schema.")
        }
        self.schema = schema
        self.rootName = root
    }
    ...
    func validate(_ source: Data, root: AgentXMLElement) throws {
        guard root.name == rootName else {
            throw AgentOutputError.invalidOutput("XML root does not match the selected schema declaration.")
        }
        guard let doc = Self.readDocument(source) else {
            throw AgentOutputError.invalidOutput("Could not read the completed XML document.")
        }
        defer { xmlFreeDoc(doc) }
        guard let context = xmlSchemaNewValidCtxt(schema) else {
            throw AgentOutputError.invalidOutput("Could not allocate XSD validation context.")
        }
        ...
        guard xmlSchemaValidateDoc(context, doc) == 0 else {
            throw AgentOutputError.invalidOutput("XML document failed XSD validation.")
        }
    }
    ...
    static func parse(_ source: Data, limits: AgentStructuredOutputLimits) throws -> AgentXMLElement {
        ...
        else { throw AgentOutputError.invalidOutput("XML must be bounded UTF-8.") }
        let parser = try AgentXMLParserEngine(
            limits: limits, options: .init(completedElements: .none, emitTextDeltas: false))
    ...
    private static func rejectExternalSchemaReferences(_ element: AgentXMLElement) throws {
        if element.attributes.contains(where: {
            $0.key.namespaceURI == "http://www.w3.org/2007/XMLSchema-versioning" && $0.key.localName == "minVersion"
                && $0.value != "1.0"
        }) {
        ...
        if element.name.namespaceURI == "http://www.w3.org/2001/XMLSchema",
            ["include", "import", "redefine", "override", "assert", "alternative", "openContent", "defaultOpenContent"]
    ...
    private static func readDocument(_ data: Data) -> xmlDocPtr? {
        data.withUnsafeBytes {
            xmlReadMemory(
                $0.baseAddress?.assumingMemoryBound(to: CChar.self), Int32(data.count), nil, "UTF-8",
