...
public struct AgentXMLResponseFormat: AgentOutputFormat {
    ...
    private let schemaSource: Result<String, any Error>

    public init(
        name: String, description: String? = nil, schema: XMLSchema,
        ...
        schemaSource = Result { try schema.xsd() }
    }
    public var codecIdentifier: String { "codexkit.xml/v1" }
    public var schemaRepresentation: String? {
        get throws {
            let source = try schemaSource.get()
            return JSONValue.object([
                "xsd": .string(source),
                "root": .string(schema.rootName.expandedName),
    ...
    public var persistence: AgentOutputPersistence<AgentXMLDocument>? {
        ...
        return .init(
            encode: { Data($0.rawXML.utf8) },
            decode: { bytes in
                try limits.validate()
                let validator = try AgentXMLSchemaValidator(
                    xsd: schemaSource.get(), root: schema.rootName, limits: limits)
                let parser = try AgentXMLParserEngine(
                    limits: limits,
                    options: .init(
                        completedElements: .none, emitTextDeltas: false, identityAttribute: options.identityAttribute))
                ...
                guard let root = parser.root, String(data: bytes, encoding: .utf8) != nil else {
                    throw AgentOutputError.invalidOutput("Invalid stored XML.")
                }
    ...
    public func makeDecoder() throws -> AgentXMLOutputDecoder {
        try limits.validate()
        let validator = try AgentXMLSchemaValidator(xsd: schemaSource.get(), root: schema.rootName, limits: limits)
...
public actor AgentXMLOutputDecoder: AgentOutputDecoder {
    ...
    init(validator: AgentXMLSchemaValidator, limits: AgentStructuredOutputLimits, options: AgentXMLStreamingOptions)
        throws
    {
    ...
    public func consume(_ bytes: Data, into sink: AgentOutputEventSink<Event>) async throws {
        ...
        guard bytes.count <= limits.maximumInputBytes - source.count else {
            throw AgentOutputError.limit("XML input exceeds byte limit.")
        }
        source.append(bytes)
        for offset in stride(from: 0, to: bytes.count, by: 64) {
