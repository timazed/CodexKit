...
struct XMLSchemaCompiler {
    ...
    static func compile(_ schema: XMLSchema) throws -> String {
        ...
        case let .document(root, types):
            guard root.occurs == .once, types.count <= 1_000 else { throw AgentOutputError.invalidFormat("Invalid root occurrence or type count.") }
            var compiler = Self(namespace: root.name.namespaceURI)
            var result = "<xs:schema xmlns:xs=\"http://www.w3.org/2001/XMLSchema\" elementFormDefault=\"unqualified\" attributeFormDefault=\"unqualified\""
            if let ns = root.name.namespaceURI { result += " targetNamespace=\"\(escape(ns))\" xmlns:t=\"\(escape(ns))\"" }
            result += ">"
            for name in types.keys.sorted() {
                try validName(name)
                switch types[name]! {
                case let .simple(type): result += try compiler.simple(type, name: name, depth: 0)
                case let .complex(content, attributes): result += try compiler.complex(content, attributes: attributes, name: name, depth: 0)
                }
            }
    ...
    static func validName(_ value: String) throws {
        guard !value.isEmpty, value.utf8.count <= 1_024, !value.utf8.contains(0),
              value.utf8CString.withUnsafeBytes({ xmlValidateNCName($0.baseAddress?.assumingMemoryBound(to: xmlChar.self), 0) }) == 0 else {
    ...
    func form(_ name: XMLName) throws -> String {
        ...
        guard name.namespaceURI == nil || name.namespaceURI == namespace else {
            throw AgentOutputError.invalidFormat("The Swift XML schema supports one target namespace; use self-contained XSD for other constructs.")
        }
    ...
    func occurrence(_ value: XMLOccurrence) throws -> String {
        ...
        guard bounds.0 >= 0, bounds.1.map({ $0 >= bounds.0 }) ?? true else { throw AgentOutputError.invalidFormat("Invalid XML occurrence bounds.") }
    ...
    mutating func element(_ value: XMLElementDeclaration, global: Bool, depth: Int) throws -> String {
        ...
        if case let .type(name) = value.content {
            guard value.attributes.isEmpty else { throw AgentOutputError.invalidFormat("Type references cannot add inline attributes.") }
            start += " type=\"\(try qualified(name))\""
        }
        start += ">"
        if let description = value.description { start += "<xs:annotation><xs:documentation>\(Self.escape(description))</xs:documentation></xs:annotation>" }
    ...
    mutating func model(_ value: XMLContentModel, depth: Int) throws -> String {
        ...
        switch value {
        case let .sequence(children, count): tag = "sequence"; elements = children; occurs = count
        case let .choice(children, count): tag = "choice"; elements = children; occurs = count
        case let .all(children, count): tag = "all"; elements = children; occurs = count
        }
    ...
    mutating func attributes(_ values: [XMLName: XMLAttribute], depth: Int) throws -> String {
        ...
        for name in values.keys.sorted(by: { $0.expandedName < $1.expandedName }) {
            ...
            switch values[name]! {
            case let .required(value): type = value; use = "required"
            case let .optional(value): type = value; use = "optional"
            }
    ...
    mutating func complex(_ content: XMLContent, attributes values: [XMLName: XMLAttribute],
                          name: String? = nil, depth: Int) throws -> String {
        try budget(depth)
        ...
        case let .text(type):
            ...
            if facets.isEmpty { result += "<xs:simpleContent><xs:extension base=\"\(base)\">\(attrs)</xs:extension></xs:simpleContent>" }
            else {
        ...
        case let .type(base): result += "<xs:complexContent><xs:extension base=\"\(try qualified(base))\">\(attrs)</xs:extension></xs:complexContent>"
        }
    ...
    mutating func simple(_ type: XMLSimpleType, name: String? = nil, depth: Int) throws -> String {
        ...
        return "<xs:simpleType" + (name.map { " name=\"\($0)\"" } ?? "") + "><xs:restriction base=\"\(base)\">\(facets)</xs:restriction></xs:simpleType>"
    }
    mutating func restriction(_ type: XMLSimpleType, depth: Int) throws -> (String, String) {
        try budget(depth)
        switch type {
        case let .stringValues(values):
            if let values, values.isEmpty { throw AgentOutputError.invalidFormat("XML enumeration must contain at least one value.") }
    ...
    mutating func facet(_ facet: XMLFacet) throws -> String {
        ...
        switch facet {
        case let .minLength(v): name = "minLength"; value = String(v)
        case let .maxLength(v): name = "maxLength"; value = String(v)
        case let .length(v): name = "length"; value = String(v)
        case let .minInclusive(v): name = "minInclusive"; value = v
        case let .maxInclusive(v): name = "maxInclusive"; value = v
        case let .minExclusive(v): name = "minExclusive"; value = v
        case let .maxExclusive(v): name = "maxExclusive"; value = v
        case let .enumeration(v): name = "enumeration"; value = v
        }
