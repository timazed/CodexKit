...
final class AgentXMLParserEngine: @unchecked Sendable {
    ...
    init(limits: AgentStructuredOutputLimits, options: AgentXMLStreamingOptions) throws {
        self.limits = limits; self.options = options
        ...
        sax.startElementNs = { ctx, local, prefix, uri, count, namespaces, attrCount, _, attributes in
            engine(ctx)?.start(local: local, prefix: prefix, uri: uri, namespaceCount: count,
        ...
        sax.resolveEntity = { ctx, _, _ in engine(ctx)?.fail("External entity resolution is prohibited."); return nil }
        ...
        sax.serror = { ctx, error in
            ...
            engine(ctx)?.fail("XML syntax error at line \(error.pointee.line), column \(error.pointee.int2) (code \(error.pointee.code)).")
        }
    ...
    func feed(_ bytes: Data, final: Bool = false) throws -> [(AgentXMLOutputEvent, Int)] {
        ...
        let result = bytes.withUnsafeBytes { pointer in
            xmlParseChunk(parser, pointer.baseAddress?.assumingMemoryBound(to: CChar.self), Int32(bytes.count), final ? 1 : 0)
        }
        if final { finished = true }
        if let error { throw error }
        guard result == 0 else { throw AgentOutputError.invalidOutput("Malformed or incomplete XML (code \(result)).") }
        if let version = parser.pointee.version, xmlString(version) != "1.0" { throw AgentOutputError.invalidOutput("Only XML 1.0 is supported.") }
        ...
        events.removeAll(keepingCapacity: true); eventBytes = 0
        return resultEvents
    }
    ...
    func emit(_ event: AgentXMLOutputEvent, size: Int) {
        ...
        guard size <= limits.maximumSemanticUnitBytes, events.count < 256,
              size <= limits.maximumQueuedEventBytes - eventBytes else { fail("XML callback batch exceeds limit."); return }
        events.append((event, size)); eventBytes += size
    }
    func start(local: UnsafePointer<xmlChar>?, prefix: UnsafePointer<xmlChar>?, uri: UnsafePointer<xmlChar>?,
        ...
        guard stack.count < limits.maximumNestingDepth, nodeCount < limits.maximumSemanticUnits,
              attributeCount >= 0, attributeCount <= 1_024, namespaceCount >= 0, namespaceCount <= 1_024 else {
            fail("XML depth, element, attribute, or namespace limit exceeded."); return
        }
        ...
        for i in 0 ..< Int(attributeCount) {
            guard let attributes, let start = attributes[i * 5 + 3], let end = attributes[i * 5 + 4] else { fail("Invalid XML attribute."); return }
            let attr = XMLName(xmlString(attributes[i * 5]), namespaceURI: optionalXMLString(attributes[i * 5 + 2]))
            guard attrs[attr] == nil else { fail("Duplicate expanded XML attribute name."); return }
            // With entity substitution disabled, SAX2 protects ampersands as
            // &#38;. Decode exactly this one layer, never arbitrary entity text.
            attrs[attr] = String(decoding: UnsafeBufferPointer(start: start, count: start.distance(to: end)), as: UTF8.self)
        ...
        path.append(.init(name: name, index: index)); nodeCount += 1
        let info = AgentXMLElementInfo(id: nodeCount, name: name,
        ...
        guard size <= limits.maximumOutputBytes - retainedBytes else { fail("XML tree exceeds retained byte limit."); return }
        retainedBytes += size
    ...
    func characters(_ text: UnsafePointer<xmlChar>?, count: Int32) {
        ...
        guard Int(count) <= limits.maximumOutputBytes - retainedBytes else { fail("XML text exceeds retained byte limit."); return }
        ...
        } else { stack[index].content.append(.text(value)) }
    ...
    func end() {
        ...
        else { stack[stack.count - 1].content.append(.element(element)) }
    }
}
