...
public struct AgentRecordResponseFormat<Record: Codable & Sendable>: AgentOutputFormat {
    ...
    public init(name: String, record: Record.Type, schema: JSONSchema? = nil, codec: AgentRecordCodec = .jsonLines,
        ...
        self.name = name; self.schema = schema; self.codec = codec; self.limits = limits
        self.minimumRecords = minimumRecords; self.maximumRecords = maximumRecords; self.finalValidation = finalValidation
    }
    ...
    public func makeDecoder() throws -> AgentJSONLinesDecoder<Record> {
        ...
        guard minimumRecords >= 0, maximumRecords >= minimumRecords,
              maximumRecords <= limits.maximumSemanticUnits else { throw AgentOutputError.invalidFormat("Invalid record count bounds.") }
    ...
    public func validateFinal(_ output: AgentRecordCollection<Record>) async throws { try await finalValidation(output) }
}
...
public actor AgentJSONLinesDecoder<Record: Codable & Sendable>: AgentOutputDecoder {
    ...
    init(schema: JSONSchema?, limits: AgentStructuredOutputLimits, minimum: Int, maximum: Int) {
        self.schema = schema; self.limits = limits; self.minimum = minimum; self.maximum = maximum
    }
    ...
    public func consume(_ bytes: Data, into sink: AgentOutputEventSink<Event>) async throws {
        ...
        guard bytes.count <= limits.maximumInputBytes - inputBytes else { throw AgentOutputError.limit("Record stream exceeds input limit.") }
        inputBytes += bytes.count
        for byte in bytes {
            try Task.checkCancellation()
            if byte == 10 { try await completeLine(into: sink) }
            else {
                guard line.count < limits.maximumSemanticUnitBytes else { throw AgentOutputError.limit("Record exceeds byte limit.") }
                line.append(byte)
            }
        }
    }
    ...
    private func completeLine(into sink: AgentOutputEventSink<Event>) async throws {
        ...
        do {
            ...
            guard encodedCount <= limits.maximumSemanticUnitBytes,
                  encodedCount + 1 <= limits.maximumOutputBytes - outputBytes else {
        ...
        catch { throw AgentOutputError.invalidOutput("Record \(records.count): \(error.localizedDescription)") }
    }
    ...
    public func cancel() { ended = true; line.removeAll(); records.removeAll() }
}
