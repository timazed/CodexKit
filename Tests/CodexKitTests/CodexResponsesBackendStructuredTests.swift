...
extension CodexResponsesBackendTests {
    ...
    func testBackendStripsStructuredStreamFramingFromVisibleAssistantText() async throws {
        ...
        """.utf8), inspect: { request in
            ...
            XCTAssertTrue(developerText.contains("Append exactly one hidden JSON block after visible text."))
            XCTAssertTrue(developerText.contains("Do not mention hidden tags/JSON visibly."))
            XCTAssertTrue(developerText.contains("Match schema shipping_reply_draft:"))
