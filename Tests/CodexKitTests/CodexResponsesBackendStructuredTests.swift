...
extension CodexResponsesBackendTests {
    ...
    func testBackendStripsStructuredStreamFramingFromVisibleAssistantText() async throws {
        ...
        """.utf8), inspect: { request in
            ...
            XCTAssertTrue(developerText.contains("Append exactly one hidden JSON block after the visible reply."))
            XCTAssertTrue(developerText.contains("Visible text first."))
            XCTAssertTrue(developerText.contains("JSON must match schema."))
            XCTAssertTrue(developerText.contains("Schema shipping_reply_draft:"))
            XCTAssertTrue(developerText.contains("<codexkit-structured-output>"))
            XCTAssertFalse(developerText.contains("CodexKit streamed response contract"))
        }))
