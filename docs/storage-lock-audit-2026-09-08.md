...
The later [hosted verification](https://github.com/timazed/CodexKit/actions/runs/34195469609) passed current and minimum-system checks, including macOS 14 and actual iOS 17.0.1 execution. Live-provider acceptance still requires a saved SDK session. The new workload exercises cooperative cancellation and orderly reopening; abrupt process termination during a commit remains outside its scope. These are coverage gaps, not additional confirmed bugs.
