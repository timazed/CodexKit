...
- SDK compatibility, current SDK tests, signed iOS/macOS demos, and the portable iOS 17 build execute in parallel. The iOS 17 runtime step reuses its signed artifact without recompilation.
- Dependency and build caches include toolchain/platform and lockfile identities. Every build/test still executes after cache restoration. The redundant standalone optimized build was folded into optimized tests, which compile all four library dependencies.
...
- Ordinary optimized concurrency uses six rounds. Relevant changes, manual extended dispatch, and scheduled verification retain 40-round stress and larger workloads.
...
| Gate/planner/demo-harness tests | 34 tests passed, including dispatch-once, timeout, stale/foreign proof, and failed-verification rejection |
