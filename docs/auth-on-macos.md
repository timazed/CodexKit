...
Alpha.33 shares token metadata resolution across local discovery, OAuth, device-code sign-in, and refresh. App-owned restoration repairs saved unknown metadata locally; external stores remain read-only. See [account metadata compatibility](account-metadata-compatibility.md) for plan cases, precedence, and identity checks. The demo sidebar shows plan, name, email, and account ID from the current SDK session.
