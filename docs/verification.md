...
Use `--build-only` to produce the signed universal app without accessing simulators, then use `--app /absolute/path/AssistantRuntimeDemoApp.app --runtime 17.0.1` on a host with the required runtime. The two modes are mutually exclusive. The seven harness tests cover mode separation, portable build settings, failure-detail retention, runtime selection, and stale/incomplete/failed report rejection.
